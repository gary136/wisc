 ...
 * 
 * TODO: Complete the implementation of the three methods below with respect to the details provided
...
public class AstraZenecaVaccine {
  ...
  public static int addRecipientToFront(String[] recipients, int size, String newRecipient,
    ...
    // TODO Complete the implementation of this method
    if (age>55) throw new IllegalArgumentException("the argument is invalid");
    if (size==recipients.length) {System.out.println("Warning! Full Array!"); return size;}
    String[] copyRecipients = new String[recipients.length+1];
    size++;
    for (int i=1; i<size; i++) {
      copyRecipients[i] = recipients[i-1];
      }
    copyRecipients[0] = newRecipient;
    for (int i=0; i<size; i++) recipients[i] = copyRecipients[i];
//    System.out.println(size);
    return size; // CHANGE this (added to avoid compile errors)
  }
   ...
   */
  public static int giveVaccineDoseToLast(String[] recipients, int size) throws IllegalStateException{
    // TODO Complete the implementation of this method
    if (dosesCount==0) throw new IllegalStateException("no doses");
    if (size==0 || recipients.length==0) {System.out.println("Error! No registered recipients in the list."); return size;}
    size--;
    vaccinated.add(recipients[size]);
  ...
  public static boolean testAddRecipient() {
    ...
    try {
      ...
      size = addRecipientToFront(recipients, size, "Emma", 75); // IllegalArgumentException expected
                                                                // to be thrown
      System.out
