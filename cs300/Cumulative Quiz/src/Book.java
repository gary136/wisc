...
class Book implements Comparable<Book>{
  ...
  public Book(String name, int pages, int chapters) throws IllegalStateException{
    ...
    if (pages<=0 || chapters<=0) throw new IllegalStateException("invalid argument");
  ...
  public String toString() {
    ...
    return this.name + ": "+ this.pages + "p/" + this.chapters + "c"; // change this
  }
  ...
  public int compareTo(Book otherBook) {
    float comparisonVal;         // Outcome of comparison
    float thisAvg = (float)this.pages/(float)this.chapters;
    float otherAvg = (float)otherBook.pages/(float)otherBook.chapters;
    comparisonVal=thisAvg - otherAvg;
//    if (this.pages/this.chapters < otherBook.pages/otherBook.chapters) {
//      
//    }
    if (comparisonVal>0) { 
