...
int main() {
...

  {
    ...
    // std::cout << "rid(recordId of [world!] is at page " << rid.page_number 
      // << ", slot " << rid.slot_number << "\n";
    new_file.writePage(third_page);

    // Retrieve the record we just added to the third page.
    std::cout << "Third page is at page " << rid.page_number 
      << ", having a new record: " << third_page.getRecord(rid) << "\n\n";
  }
...
void test1(File &file1) {
  ...
  for (i = 0; i < num; i++) {
    // pid[i] = i
    // pid[i] = page->page_number(); // increment pageNo
    // std::cout << "i=" << i << ", pageNo bfr=" << pid[i] << ", page_number()=" << page->page_number() << "\n";
    bufMgr->allocPage(file1, pid[i], page); // page is pointer here
    // // pageNo is assigned in allocPage
    // std::cout << "pageNo after allocation=" << pid[i] 
    //   << ", page_number()=" << page->page_number() << "\n";       
    std::cout << "page's nmbr: " << page->page_number() << "|" << pid[i] << "\n";
    sprintf(tmpbuf, "test.1 Page %u %7.1f", pid[i], (float)pid[i]);
    // std::cout << "tmpbuf: " << tmpbuf << "\n";
    // std::cout << "done allocPage\n";
    rid[i] = page->insertRecord(tmpbuf);
    // std::cout << "done addressing insertRecord\n";
    // std::cout << "page " << rid[i].page_number 
    //   << " has a new record: " << page->getRecord(rid[i]) << "\n";
    bufMgr->unPinPage(file1, pid[i], true); 
    // std::cout << "done unPinPage\n\n";
    std::cout << "\n";
  }

  std::cout << "done Allocating pages\n\n";
  ...
  for (i = 0; i < num; i++) {
    ...
    sprintf(tmpbuf, "test.1 Page %u %7.1f", pid[i], (float)pid[i]);
    // std::cout << "tmpbuf: " << tmpbuf << "\n";
    // std::cout << "page->getRecord(rid[i]): " << page->getRecord(rid[i]) << "\n";
...
void test2(File &file1, File &file2, File &file3) {
  ...
  for (i = 0; i < num / 3; i++) {
    // alloc page 2 and insert a record
    bufMgr->allocPage(file2, pageno2, page2);    
    std::cout << "page2's nmbr: " << page2->page_number() << "|" << pageno2 << "\n";
    sprintf(tmpbuf, "test.2 Page %u %7.1f", pageno2, (float)pageno2);
    rid2 = page2->insertRecord(tmpbuf);
    // std::cout << "tmpbuf: " << tmpbuf << "\n";
    std::cout << "a\n";
    // std::cout << "a: record of page 2: " << page2->getRecord(rid2).c_str() << "\n\n";
    
    // read a random page and check if content is right
    ...
    // std::cout << "b\n";
    std::cout << "b: record of page 2: " << page2->getRecord(rid2).c_str() << "\n\n";

    // alloc page 3 and insert a record
    ...
    // std::cout << "c\n";    
    std::cout << "c: record of page 2: " << page2->getRecord(rid2).c_str() << "\n\n";

    // read page 2 and check if content is right
    bufMgr->readPage(file2, pageno2, page2);
    sprintf(tmpbuf, "test.2 Page %u %7.1f", pageno2, (float)pageno2);
    // update
    std::cout << "d: record of page 2: " << page2->getRecord(rid2).c_str() << "\n";
    if (strncmp(page2->getRecord(rid2).c_str(), tmpbuf, strlen(tmpbuf)) != 0) {
      PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH");
    }
    // std::cout << "d\n";
    
    // read page 3 and check if content is right
