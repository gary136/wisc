...
void createRelationWithExtremeValue();
void intTests();
void intTests2();
...
void test4()
{
	...
	// on attributes of all three types (int, double, string)
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationWithExtremeValue" << std::endl;
	createRelationWithExtremeValue();
	intTests2();
	try
	{
...
void createRelationWithExtremeValue()
{
  ...
  // Insert a bunch of tuples into the relation.
  int testValues[3] = {INT32_MAX, INT32_MIN, 0};
  for(int i = 0; i < 3; i++ )
	{
    sprintf(record1.s, "%05d string record", i);
    record1.i = i;
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,3000,GTE,8000,LT), 2000)
	for (int i = 0; i < 10000; ++i) {
		int l = random() % 5000;
		int r = random() % 5000;
		if (l > r) std::swap(l, r);
		checkPassFail(intScan(&index,l,GTE,r,LT), r - l);
	}
}
...
void intTests2()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	// run some tests
	checkPassFail(intScan(&index,INT32_MIN,GTE,INT32_MAX,LTE), 3)
}
