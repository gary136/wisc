...
static_assert(sizeof(IndexMetaInfo) <= sizeof(Page));
...
struct NonLeafNodeInt{
  ...
  /**
   * number of valid keys in this node
   */
	int numValidKeys;
};

static_assert(sizeof(NonLeafNodeInt) == sizeof(Page));
...
static_assert(sizeof(LeafNodeInt) == sizeof(Page));
...
class BTreeIndex {
 ...
 private:
  ...
  /**
   * The page number of IndexMetaInfo
   */
  static constexpr PageId indexMetaInfoPage = 1;


  /**
   * Write the IndexMetaInfo to file.
   */
  void writeMetaInfo(bool alloc = false);

  PageId searchLeaf(int entry, std::vector<PageId> *parents);
